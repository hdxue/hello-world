		...
		webBrowser.addWebBrowserListener(new WebBrowserAdapter() {
			...
			public void loadingProgressChanged(WebBrowserEvent e) {
				...
				if (e.getWebBrowser().getLoadingProgress() == 100) {
					...
					// 当网页超出目标大小时
					// if (imageSize.width > maxWidth || imageSize.height >
					// maxHeight) {
					// 截图部分图形
					image = image.getSubimage(0, 0, imageSize.width, imageSize.height);
					...
					// AffineTransformOp.TYPE_NEAREST_NEIGHBOR); // 缩小
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				frame.getContentPane().add(new Main("http://www.junletk.cc/aboutus.htm", 640, 480), BorderLayout.CENTER);
